...
import { fetchInstance, fetchInstanceWithToken } from '../../utils/fetchInstance';
...
const Component: FC = () => {
  ...
  const { data, error } = useSWR<any>(`/api/timelines`, fetchInstanceWithToken());
    ...
    <>
      {timelines.map((timeline: any) => (
        <Card key={timeline.id}>
          <div className="w-full h-full py-2" onMouseEnter={() => handleFootprints(timeline.id, timeline.attributes.footprints)}>
            {/* user info  */}
            <div className="flex justify-start items-center gap-2">
              <Icon icon="ant-design:user-outlined" className="w-10 h-10" />
              <div className="flex flex-col overflow-hidden whitespace-nowrap">
                <p className="text-md font-semibold">{timeline.attributes.institution.data.attributes.name}</p>
                <p className="text-sm text-gray-500">{timeline.attributes.user.data.attributes.email}</p>
              </div>
            </div>
            {/* post content */}
            <div className="w-full h-full flex flex-col md:flex-row gap-4 px-0 md:px-12 py-2">
              {timeline.attributes.images.data.length > 0 &&
                <div className={`w-full md:w-2/3 grid gap-2 ${timeline.attributes.images.data.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                  {timeline.attributes.images.data.map((image: any) => (
                    <div key={image.id} className="flex justify-center items-center">
                      <img
                        ...
                        alt="timeline"
                        onClick={() => handleOpen(`${process.env.REACT_APP_API_URL}${image.attributes.url}`)}
                      />
                      {isOpen && (
            ...
            <div className="flex justify-between items-center gap-4 px-0 md:px-12 py-2">
              <div className="flex justify-center items-center gap-2">
                <Icon icon="ant-design:clock-circle-outlined" className="w-4 h-4" />
                <p className="text-sm text-gray-500">{new Date(timeline.attributes.createdAt).toLocaleString()}</p>
              </div>
              <div className="flex justify-center items-center gap-2">
                <Icon icon="fluent-emoji-high-contrast:paw-prints" className="w-4 h-4" />
                <p className="text-sm text-gray-500">{timeline.attributes.footprints}</p>
              </div>
            </div>
          </div>
        </Card>
